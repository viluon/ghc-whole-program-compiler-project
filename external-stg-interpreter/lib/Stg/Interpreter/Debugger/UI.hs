...
debugProgram :: Bool -> String -> String -> [String] -> DebuggerChan -> Unagi.InChan DebugCommand -> Unagi.OutChan DebugOutput -> Maybe String -> IO ()
debugProgram switchCWD appPath cbitsPath appArgs dbgChan dbgCmdI dbgOutO dbgScript = do
  case dbgScript of
    Just fname -> do
  ...
  loadAndRunProgram switchCWD appPath cbitsPath appArgs dbgChan DbgStepByStep True
